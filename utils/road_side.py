...
def count_angle_line(x1, y1, x2, y2):
    angle = abs(math.atan2(abs(y2 - y1), abs(x2 - x1)) * 180.0 / math.pi)
    return angle


def is_near_center_line(width, x1, x2):
    center = width / 2
...
def detect_road_side(img):
    ...
    for line in lines:
        x1, y1, x2, y2 = line[0]
        angle = count_angle_line(x1, y1, x2, y2)
        ...
        multiplier = 1.5 if is_near_center_line(width, x1, x2) else 1
